...
<form id="editMemberForm" class="member-edit-form" enctype="multipart/form-data" data-member-id="<?= (int) $member['id'] ?>">
    ...
    <div class="card mb-3"><div class="card-body">
        ...
        <div class="row g-3 mb-3">
            <div class="col-12 col-md-6"><label class="form-label">Gender</label><select name="gender" class="form-select"><option value="male" <?= $member['gender']==='male'?'selected':'' ?>>Male</option><option value="female" <?= $member['gender']==='female'?'selected':'' ?>>Female</option></select></div>
            <div class="col-12 col-md-6"><label class="form-label">Status</label><select name="status" class="form-select"><?php foreach (['active','pending','suspended','expired'] as $s): ?><option value="<?= $s ?>" <?= $member['status']===$s?'selected':'' ?>><?= ucfirst($s) ?></option><?php endforeach; ?></select></div>
        </div>
        ...
        <div class="mb-3"><label class="form-label">Membership Type</label><select name="membership_type_id" class="form-select"><?php foreach ($membershipTypes as $t): ?><option value="<?= $t['id'] ?>" <?= $member['membership_type_id']==$t['id']?'selected':'' ?>><?= \App\Core\View::escape(\App\Helpers\MembershipType::optionLabel($t)) ?></option><?php endforeach; ?></select></div>
